...
- **Preview channel:** Settings → **Preview updates** sets `allowPrerelease` in `desktop-state.json` (default off). Stable installers ignore `vX.Y.Z-beta.N` GitHub prereleases. Testers turn the switch on, then Check for updates. Do **not** tag a hyphen-free `vX.Y.Z` for a test flight — that updates everyone.
...
- **Home jail** — tool shells cannot read/write the rest of `$HOME` (`.ssh` private keys, sibling repos, Docker config). **`GROK_HOME` (`~/.grok`) is always bound** so skills, agents, personas, and sessions work with sandbox on. Other global host paths still need sandbox off or “Allow outside project”.
- **Git config** — Seatbelt EPERM on an existing `~/.gitconfig` is fatal to git (`unable to access … Operation not permitted`). Jailed terminals set `GIT_CONFIG_GLOBAL=/dev/null`, `GIT_CONFIG_SYSTEM=/dev/null`, `GIT_CONFIG_NOSYSTEM=1`, and copy host `user.name` / `user.email` into `GIT_AUTHOR_*` / `GIT_COMMITTER_*` / `GIT_CONFIG_COUNT`. Seatbelt also allows **read** of the usual git identity files (`~/.gitconfig`, `~/.config/git/{config,ignore,attributes}`, `~/.gitignore_global`); writes stay denied. Do not bind `~/.git-credentials` or `~/.ssh` (private keys stay denied).
- **SSH host keys** — Jail still denies `~/.ssh` (no `id_*`, no bind of the directory). Host keys live under `GROK_HOME/ssh/known_hosts` (writable in the jail). Desktop copies `~/.ssh/known_hosts` into that file at spawn. `GIT_SSH_COMMAND` uses `StrictHostKeyChecking=accept-new` + `UserKnownHostsFile` under `GROK_HOME/ssh` so first `git fetch` records the key and later connects stay strict. `SSH_AUTH_SOCK` is passed through for agent auth.
