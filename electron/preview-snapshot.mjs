...
/**
 * @param {{ url?: string, title?: string, headings?: string[], nodes?: Array<Record<string, unknown>>, truncated?: boolean }} raw
 */
export function formatPreviewSnapshot(raw) {
  ...
  if (!nodes.length) {
    ...
    for (const n of nodes) {
      ...
      lines.push(name ? `  - ${ref}${kind} "${name}"` : `  - ${ref}${kind}`);
    }
  }
  lines.push(
    "Interact: desktop-preview__preview_click / preview_fill / preview_fill_form / preview_press / preview_interact. Never PowerShell or curl.",
  );
  if (raw?.truncated) {
