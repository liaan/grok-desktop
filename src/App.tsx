...
export default function App() {
  ...
  const sendPrompt = async () => {
    ...
    if ((!text && images.length === 0) || !project || busyRef.current) return;
    ...
    setItems((prev) => [
      ...prev,
      {
        id: uid("user"),
        kind: "user",
        text: text || (images.length ? `(${images.length} image${images.length > 1 ? "s" : ""})` : ""),
      ...
      <main className="main">
        ...
        <div className="composer">
          ...
          >
            ...
            <textarea
              value={input}
              placeholder="Ask Grok to build, fix, or explain… (Paste or drop images · Enter to send · Shift+Enter for newline)"
