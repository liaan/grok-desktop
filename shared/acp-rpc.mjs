...
/**
 * grok-build `x.ai/interject` (extensions/interject.rs). Text-only omits
 * `content` so the legacy wire shape stays byte-identical to the TUI.
 *
 * @param {{
 *   sessionId: string,
 *   text: string,
...
/**
 * Wire methods for `grok agent stdio` mid-turn interject.
 * @param {{
 *   sessionId: string,
 *   text: string,
 *   interjectionId: string,
...
/**
 * Agent→client echo after `x.ai/interject` (`x.ai/session/interjection`).
 * @param {unknown} method
 */
 ...
 * @param {unknown} method
 * @param {any} params
 * @returns {{ sessionId: string, text: string, interjectionId: string } | null}
 */
